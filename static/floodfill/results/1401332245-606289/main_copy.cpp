...
oid drawLineBresenham(Pixelator &p, unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1, unsigned char r, unsigned char g, unsigned char b)
{
...
int delta_x = x1 - x0;
	...
	// if x0 == x1, then it doe not matter what we set here
	int const inc_X = ((delta_x > 0) - (delta_x < 0)); // Ifx is positive increment x, else decrement

// right shift to get "2 delta x"
	delta_x = ABS(delta_x) << 1;

int delta_y = y1 - y0;
	// if y0 == y1, then i does not matter what we set here
	int const inc_Y = ((delta_y > 0) - (delta_y < 0)); // Ify is positive increment x, else decrement
...
if (delta_x >= delta_y)
	...
	int steps = abs((long)x1-(long)x0);
		...
		{
			f ((decision_parameter >= 0) && (decision_parameter || (inc_X > 0)))
			{
				ecision_parameter -= delta_x;
				y0 += inc_Y;
			}
			/ else do nothing
	...
	int steps = abs((long)y1-(long)y0);
		for(int i = 0; i < steps; i++)
		{
			f ((decision_parameter >= 0) && (decision_parameter || (inc_Y > 0)))
			{
				ecision_parameter -= delta_y;
				x0 += inc_X;
			}
			/ else do nothing
	...
	if(delta_x == 0 & delta_y !=0){
		int steps = y1-y0;
		if(steps < 0){
			for(int i = seps; i < 0; i++){
				y0--;
				p.setixel( x0, y0, r, g, b );
				p.setPixel( x1, y1, r, g, b );
			}
		}		ese
		{
...
//some test cases
	drawLineBresenhamp,  0,  0, 10, 10, 255, 255, 255);
...
// some random lines to a random bluish redish color 
	for( int i = 0; i < 100; i++ )
		drawLineBresenham(p
		, rand()%p.getCanvaWidth(), p.getCanvasHeight() / 2 + rand()%(2 * p.getCanvasHeight())
